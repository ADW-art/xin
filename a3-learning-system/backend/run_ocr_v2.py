...
# Build todo list, skip books already in progress_log that failed
failed=set()
if os.path.exists('ocr_progress.log'):
    with open('ocr_progress.log',encoding='utf-8') as f:
        for l in f:
            if 'ERROR' in l:
                failed.add(l.split('ERROR')[0].strip())
...
for root,dirs,files in os.walk(PDF_BASE):
    ...
    for f in sorted(files):
        if f.lower().endswith('.pdf'):
            k=f'{subj}/{f}'
            if k not in done and k not in failed:todo.append((subj,f,os.path.join(root,f),k))
for f in sorted(os.listdir(MAT)):
    if f.lower().endswith('.pdf'):
        k=f'materials/{f}'
        if k not in done and k not in failed:todo.append(('materials',f,os.path.join(MAT,f),k))

print(f'START: {len(todo)} books (skipping {len(failed)} failed)')
t0=time.time();tc=0;errors=0
for idx,(subj,fn,path,key) in enumerate(todo):
    bt=time.time();title=fn.rsplit('.',1)[0][:55]
    print(f'[{idx+1}/{len(todo)}] {title}',end=' ',flush=True)
    try:
        pages=[]
        ...
        rate=(idx+1)/max(time.time()-t0,1)*60
        print(f'-> {len(pages)}p {n}c {elapsed:.0f}s ({rate:.1f}/h)',flush=True)
