...
.bkt { height: 100%; display: flex; flex-direction: column; overflow: hidden; background: #F8FAFC }

/* Header bar */
.bkt-bar { display: flex; align-items: flex-start; justify-content: space-between; padding: 20px 24px 14px; flex-shrink: 0; background: #fff; border-bottom: 1px solid #E2E8F0 }
.bkt-bar-l h1 { font-size: var(--font-xl); font-weight: 700; margin: 0; color: #1E293B }
.bkt-bar-l p { font-size: var(--font-sm); color: #64748B; margin-top: 3px }
.bkt-bar-l strong { color: #2563EB }
.bkt-bar-r { display: flex; gap: 10px; align-items: center }

/* Stats row */
.bkt-stats { display: flex; gap: 0; padding: 10px 24px; flex-shrink: 0; background: #fff; border-bottom: 1px solid #E2E8F0 }
.bs-item { flex: 1; text-align: center; padding: 4px 0 }
.bs-v { font-size: 18px; font-weight: 700; display: block; color: #1E293B }
.bs-l { font-size: 10px; color: #94A3B8; margin-top: 2px }
...
/* Cards (Blue-White) */
.bkt-card { background: #fff; border: 1px solid #E2E8F0; border-radius: 8px; overflow: hidden }
.bc-hd { padding: 12px 16px; font-size: var(--font-sm); font-weight: 600; display: flex; align-items: center; gap: 8px; color: #334155; border-bottom: 1px solid #F1F5F9 }
.bc-bd { padding: 12px 16px }
.chart-sub { font-weight: 400; color: #94A3B8; font-size: var(--font-xs) }
.mode-badge { font-size: 10px; padding: 2px 10px; border-radius: 12px; margin-left: auto; font-weight: 600; background: #EFF6FF; color: #2563EB }

/* ═══ Concept List (Blue-White) ═══ */
.concept-row { padding: 10px 0; cursor: pointer; transition: all .15s; border-bottom: 1px solid #F8FAFC }
.concept-row:last-child { border-bottom: none }
.concept-row:hover { background: #F8FAFC; margin: 0 -16px; padding: 10px 16px; border-radius: 6px }
.concept-row.active { background: #EFF6FF; margin: 0 -16px; padding: 10px 16px; border-radius: 6px }
...
.cr-name { font-size: var(--font-sm); font-weight: 500; flex: 1; min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; color: #334155 }
.cr-pct { font-size: var(--font-sm); font-weight: 700; flex-shrink: 0; margin-left: 8px; color: #2563EB }

.cr-bar-wrap { height: 6px; background: #F1F5F9; border-radius: 3px; overflow: hidden }
.cr-bar { height: 100%; border-radius: 3px; transition: width .6s ease }

.cr-meta { display: flex; align-items: center; gap: 6px; margin-top: 5px; font-size: var(--font-xs) }
.cr-tag { padding: 1px 8px; border-radius: 6px; font-weight: 600; font-size: 10px; background: #EFF6FF; color: #2563EB }
.cr-n { color: #94A3B8 }
.cr-badge { font-size: 9px; padding: 0 5px; border-radius: 5px; font-weight: 700; background: #DBEAFE; color: #1D4ED8 }

.cr-more { text-align: center; font-size: var(--font-xs); color: #94A3B8; padding: 8px 0 2px }

/* ═══ Parameters Grid (Blue-White) ═══ */
.params-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 8px; margin-bottom: 12px }
.pp-cell { background: #F8FAFC; border-radius: 8px; padding: 12px 10px; text-align: center; border: 1px solid #F1F5F9 }
.pp-k { display: block; font-size: 12px; font-weight: 700; color: #2563EB; font-family: monospace }
.pp-v { display: block; font-size: 18px; font-weight: 700; font-family: monospace; margin: 3px 0; color: #1E293B }
.pp-d { display: block; font-size: 10px; color: #94A3B8 }

/* EM compact */
.em-compact { border-top: 1px dashed #E2E8F0; padding-top: 10px }
.em-title { font-size: var(--font-xs); font-weight: 600; color: #64748B; margin-bottom: 8px; display: flex; align-items: center; gap: 6px }
.em-cnt { font-weight: 400; color: #94A3B8; background: #F1F5F9; padding: 1px 8px; border-radius: 10px; font-size: 10px }
.em-row { display: flex; align-items: center; gap: 6px; padding: 5px 0; font-size: var(--font-xs) }
.em-row:last-child { border-bottom: none }
.em-n { flex: 1; font-weight: 500; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; color: #334155 }
.em-p { font-family: monospace; color: #64748B }
.em-r { font-family: monospace; font-weight: 700; color: #2563EB }

/* Metrics inline */
.metrics-inline { display: flex; gap: 16px; margin-top: 10px; padding-top: 10px; border-top: 1px dashed #E2E8F0; font-size: var(--font-xs); color: #64748B }
.metrics-inline strong { font-weight: 600; color: #1E293B }
...
.f-card { background: #F8FAFC; border-radius: 8px; padding: 14px; border: 1px solid #E2E8F0 }
.f-correct { border-color: #DBEAFE }
.f-wrong { border-color: #FEE2E2 }
.f-title { font-size: var(--font-xs); font-weight: 600; color: #64748B; margin-bottom: 8px }
.f-frac { display: flex; flex-direction: column; align-items: center }
.f-num { border-bottom: 1.5px solid #CBD5E1; padding: 4px 8px; font-family: 'Times New Roman', serif; font-style: italic; font-size: 13px; color: #334155 }
.f-den { padding: 4px 8px; font-family: 'Times New Roman', serif; font-style: italic; font-size: 13px; color: #94A3B8 }
.f-res { margin-top: 8px; font-size: 13px; font-weight: 600; text-align: center; padding: 6px; border-radius: 5px }
.f-res.ok { background: #EFF6FF; color: #2563EB }
.f-res.err { background: #FEF2F2; color: #DC2626 }
.f-arrow { margin: 0 4px; color: #94A3B8 }
.f-delta { font-size: 11px; margin-left: 4px; color: #2563EB }
.f-delta.neg { color: #DC2626 }
.f-note { margin-top: 10px; padding: 8px 10px; background: #F8FAFC; border-radius: 6px; font-size: var(--font-xs); color: #64748B; font-family: monospace }

/* ═══ Demo (Blue-White) ═══ */
.demo-body { display: flex; align-items: stretch; gap: 16px }
.demo-col { flex: 1; text-align: center }
.demo-label { font-size: var(--font-xs); color: #94A3B8; margin-bottom: 4px }
.demo-val { font-size: 28px; font-weight: 800; font-family: monospace; line-height: 1.2; color: #2563EB }
.demo-val.after { color: #1D4ED8 }
.demo-chg { font-size: 13px; font-weight: 600; margin-left: 3px; color: #2563EB }
.demo-chg.dn { color: #DC2626 }
.demo-bar { height: 6px; background: #F1F5F9; border-radius: 3px; margin-top: 6px; overflow: hidden }
.demo-fill { height: 100%; border-radius: 3px; transition: all .35s ease; background: #2563EB }
.demo-fill.after { background: #1D4ED8; opacity: .85 }

.demo-mid { display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 6px; padding-top: 22px }
.btn-ok, .btn-ng { padding: 8px 20px; border: none; border-radius: 6px; cursor: pointer; font-size: 13px; font-weight: 600; transition: all .12s }
.btn-ok:disabled, .btn-ng:disabled { opacity: .45; cursor: not-allowed }
.btn-ok { background: #DBEAFE; color: #1D4ED8 }
.btn-ok:hover:not(:disabled) { background: #BFDBFE }
.btn-ng { background: #FEE2E2; color: #DC2626 }
.btn-ng:hover:not(:disabled) { background: #FECACA }
.demo-reset { font-size: var(--font-xs); color: #94A3B8; cursor: pointer; text-decoration: underline }

.demo-detail { margin-top: 10px; border-top: 1px dashed #E2E8F0; padding-top: 8px }
.dt-table { width: 100%; border-collapse: collapse; font-size: var(--font-xs); font-family: monospace }
.dt-table td { padding: 4px 6px; color: #64748B; border-bottom: 1px solid #F1F5F9 }
.dt-table td:last-child { font-weight: 600; color: #1E293B }
.dt-final { background: #F8FAFC; font-weight: 600 !important }

/* ═══ Loading / Empty ═══ */
.bkt-loading { flex: 1; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 12px; color: #94A3B8 }
